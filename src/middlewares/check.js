...
module.exports = {
  async email(req, res, next) {
    ...
    if (checkEmail.length)
      return next(
        createError.NotFound("Account with email address already exist")
      );
  ...
  async emailExist(req, res, next) {
    ...
    if (checkEmail.length)
      return next(
        createError.Conflict("Account with email address already exist")
      );
  ...
  async usernameExist(req, res, next) {
    ...
    if (checkUsername.length)
      return next(createError.Conflict("Account with username already exist"));
